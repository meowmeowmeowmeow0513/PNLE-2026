...
const OnboardingFlow: React.FC = () => {
  ...
  const renderStep1_Welcome = () => (
    <div className="text-center animate-in fade-in slide-in-from-bottom-8 duration-700">
      <div className="w-24 h-24 bg-gradient-to-br from-pink-500 to-rose-600 rounded-[2rem] mx-auto mb-8 shadow-[0_0_50px_-10px_rgba(236,72,153,0.6)] flex items-center justify-center transform rotate-3 ring-4 ring-white/10">
      ...
      <button 
        onClick={handleNext}
        className="px-12 py-5 bg-white text-slate-900 rounded-full font-bold text-lg hover:bg-slate-200 transition-all flex items-center gap-2 mx-auto shadow-2xl hover:scale-105 active:scale-95 group"
      >
  ...
  const renderStep3_Goal = () => (
    <div className="w-full max-w-2xl mx-auto animate-in fade-in slide-in-from-right-8 duration-500">
      ...
      <p className="text-slate-400 text-center mb-10">This calibrates your daily motivation engine.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div 
            ...
            className={`cursor-pointer group relative p-8 rounded-3xl border transition-all duration-300 ${
                goal === 'top' 
                ? 'bg-gradient-to-b from-purple-900/50 to-slate-900 border-purple-500 ring-1 ring-purple-500/50 scale-105' 
          ...
          >
            ...
            <p className="text-slate-400 text-sm leading-relaxed">I'm aiming for the Top 10. I want intense focus, strict tracking, and advanced metrics. Pressure makes diamonds.</p>
          </div>
          ...
          <div 
            ...
            className={`cursor-pointer group relative p-8 rounded-3xl border transition-all duration-300 ${
                goal === 'pass' 
                ? 'bg-gradient-to-b from-emerald-900/50 to-slate-900 border-emerald-500 ring-1 ring-emerald-500/50 scale-105' 
          ...
          >
            ...
            <p className="text-slate-400 text-sm leading-relaxed">I want a balanced, sustainable study routine. Consistent progress, mental clarity, and peace of mind.</p>
          </div>
      </div>
  ...
  const renderStep4_Manifesto = () => (
      <div className="w-full max-w-xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700 text-center">
          <Scroll size={48} className="text-amber-400 mx-auto mb-6 opacity-80" />
          <h2 className="text-3xl font-black text-white mb-8 uppercase tracking-widest">The Pledge</h2>
          
          <div className="bg-white/5 border border-white/10 rounded-2xl p-8 mb-8 backdrop-blur-sm relative overflow-hidden">
          ...
          <button 
            onClick={handleNext}
            className="px-10 py-4 bg-amber-500 hover:bg-amber-400 text-black rounded-full font-bold transition-all shadow-[0_0_30px_-5px_rgba(245,158,11,0.4)] hover:scale-105"
          >
    ...
    <div className="text-center animate-in zoom-in duration-500">
      ...
      {!loading && (
        <button 
            ...
            className="px-12 py-5 bg-pink-600 hover:bg-pink-500 text-white rounded-full font-bold shadow-[0_0_40px_-5px_rgba(236,72,153,0.6)] transition-all hover:scale-105 text-lg"
        >
            Enter Dashboard
        </button>
      )}
  ...
  return (
    <div className={`fixed inset-0 z-[100] bg-[#020617] flex flex-col font-sans selection:bg-pink-500/30 transition-opacity duration-700 ease-in-out ${isExiting ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            ...
            {[1, 2, 3, 4, 5].map((s) => (
                <div 
                    ...
                    className={`h-1.5 rounded-full transition-all duration-500 ${
                        s <= step ? 'w-8 bg-pink-500 shadow-lg shadow-pink-500/50' : 'w-2 bg-slate-800'
                    }`} 
                />
            ))}
         </div>
      ...
      <footer className="relative z-10 py-6 text-center">
          <p className="text-slate-600 text-xs uppercase tracking-widest flex items-center justify-center gap-2 font-bold">
