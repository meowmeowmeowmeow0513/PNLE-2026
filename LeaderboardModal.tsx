...
const LeaderboardModal: React.FC<LeaderboardModalProps> = ({ onClose }) => {
    ...
    const handleShare = async () => {
        ...
        try {
            ...
            if (myEntry) {
                ...
                message = `🏆 **${myEntry.displayName}** is currently **Rank #${rank}** on the Global Leaderboard!\n\n✨ **Stats:**\n• ${myEntry.totalXP.toLocaleString()} XP\n• ${myEntry.currentStreak} Day Streak\n• Title: ${myEntry.rankTitle}`;
    ...
    const getRankIcon = (index: number) => {
        ...
        return <span className="font-mono font-bold text-slate-400 dark:text-slate-500 text-base w-8 text-center">#{index + 1}</span>;
    };
    ...
    return createPortal(
        <div className="fixed inset-0 z-[9999] flex items-end md:items-center justify-center md:p-6 animate-fade-in">
            {/* Backdrop */}
            ...
            <div 
                className={`
                    relative z-10 
                    w-full h-[90dvh] md:h-auto md:max-h-[85vh] 
            ...
            >
                {/* Mobile Handle */}
                <div className="md:hidden w-full flex justify-center pt-3 pb-1 shrink-0 cursor-pointer" onClick={onClose}>
                ...
                <div className="p-6 md:p-8 bg-white/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center relative overflow-hidden shrink-0 backdrop-blur-md">
                    {/* Background Glow */}
                    <div className="absolute top-0 right-0 w-64 h-64 bg-yellow-500/10 rounded-full blur-[80px] -mr-16 -mt-16 pointer-events-none"></div>
                ...
                {/* List Content */}
                <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-50/50 dark:bg-[#0B1121] pb-8">
                    {loading ? (
                        <LoadingSkeleton />
                    ) : error ? (
