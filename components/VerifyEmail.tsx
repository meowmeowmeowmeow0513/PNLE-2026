...
const VerifyEmail: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 h-[100dvh] w-screen overflow-y-auto bg-[#F3F4F6] dark:bg-slate-950 flex flex-col items-center justify-center p-4 z-[100]">
      <div className="min-h-full w-full flex flex-col items-center justify-center py-12 relative">
        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl shadow-xl p-8 border border-slate-100 dark:border-slate-800 text-center animate-fade-in relative overflow-hidden">
          ...
          <p className="text-slate-600 dark:text-slate-300 mb-6 leading-relaxed text-sm">
            ...
            <span className="font-bold text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded">{currentUser?.email}</span>
          </p>
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              disabled={cooldown > 0}
              className="w-full py-3.5 px-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-white font-medium rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed text-sm"
            >
