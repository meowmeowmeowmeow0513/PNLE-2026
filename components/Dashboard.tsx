...
// ... (Internal Components: GhostEmptyState, StudyVitals, PNLECountdown remain unchanged) ...
    ...
    <div className="h-full w-full flex flex-col items-center justify-center relative overflow-hidden rounded-2xl bg-slate-50/50 dark:bg-[#0B1121] border border-slate-200 dark:border-white/5 group select-none transition-all duration-500 hover:shadow-lg dark:hover:shadow-pink-900/5 hover:border-pink-200 dark:hover:border-pink-500/20 py-8">
       ...
       <div className="relative z-10 animate-float mb-6 transform transition-all duration-500 group-hover:scale-110 group-hover:-translate-y-4 cursor-pointer">
           <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-xl dark:drop-shadow-[0_0_25px_rgba(255,255,255,0.15)]">
               <path d="M50 15C33.4315 15 20 28.4315 20 45V85L30 78L40 85L50 78L60 85L70 78L80 85V45C80 28.4315 66.5685 15 50 15Z" className="fill-white dark:fill-white/95 transition-colors" />
       ...
       <div className="text-center z-10 px-6 relative">
           ...
           <button onClick={() => document.getElementById('quick-add-input')?.focus()} className="mt-4 text-[10px] font-bold uppercase tracking-widest text-pink-500 hover:text-white hover:bg-pink-50 bg-white dark:bg-pink-500/5 px-6 py-2 rounded-full border border-pink-200 dark:border-pink-500/20 transition-all hover:scale-105 active:scale-95 shadow-sm hover:shadow-pink-500/30">Create First Task</button>
       </div>
       <style>{`
         @keyframes float { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-10px); } }
         @keyframes shadow { 0%, 100% { transform: translateX(-50%) scale(1); opacity: 0.3; } 50% { transform: translateX(-50%) scale(0.7); opacity: 0.15; } }
...
const StudyVitals = () => {
  ...
  if (bpm < 60) { rhythmName = "Sinus Bradycardia"; statusColor = "text-slate-400"; } else if (bpm > 100) { rhythmName = "Sinus Tachycardia"; statusColor = "text-pink-500"; }
  const pathData = useMemo(() => {
    if (total === 0 || completed === 0) return "M 0 50 L 10 50 L 15 48 L 20 52 L 25 50 L 100 50";
    const points = []; const segments = 15; const amplitude = 10 + (progress * 40);
    for (let i = 0; i <= segments; i++) { const x = (i / segments) * 100; let y = 50; if (i % 3 === 0) { y = 50 - (Math.random() * amplitude); } else if (i % 3 === 1) { y = 50 + (Math.random() * (amplitude / 2)); } points.push(`${i === 0 ? 'M' : 'L'} ${x} ${y}`); }
    return points.join(' ');
  ...
  return (
    <div className="relative w-full h-full flex flex-row items-center justify-between overflow-hidden px-6 gap-6">
        ...
        <div className="relative z-10 flex flex-col justify-center min-w-[140px]">
            ...
            <h3 className={`text-lg font-black tracking-tight whitespace-nowrap ${statusColor === 'text-slate-400' ? 'text-slate-500 dark:text-slate-400' : 'text-slate-800 dark:text-white'}`}>{rhythmName}</h3>
             <div className="flex items-baseline gap-1 mt-1">
                <span className="text-3xl font-mono font-bold text-slate-800 dark:text-white transition-all duration-1000">{bpm}</span>
        ...
        <div className="relative h-16 flex-1 mx-4 hidden sm:block">
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full overflow-visible">
                <defs><filter id="glow" x="-20%" y="-20%" width="140%" height="140%"><feGaussianBlur stdDeviation="2" result="coloredBlur" /><feMerge><feMergeNode in="coloredBlur" /><feMergeNode in="SourceGraphic" /></feMerge></filter></defs>
                <path d={pathData} fill="none" stroke={progress > 0.8 ? '#ef4444' : '#ec4899'} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" filter="url(#glow)" className="transition-all duration-1000 drop-shadow-[0_0_8px_rgba(236,72,153,0.5)]" />
            </svg>
        </div>
        <div className="relative z-10 flex flex-col items-end gap-2">
             <div className={`text-[10px] px-3 py-1 rounded-full font-bold border flex items-center gap-1.5 ${progress >= 0.5 ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-slate-500/10 text-slate-500 border-slate-500/20'}`}>
                {progress >= 0.5 ? <ArrowUpRight size={12} /> : <Activity size={12} />}
                {progress >= 0.5 ? 'High Intensity' : 'Low Activity'}
             </div>
             <span className="text-xs text-slate-400 font-mono">{completed}/{total} Tasks Done</span>
        </div>
    </div>
  );
};
...
const PNLECountdown = () => {
    ...
    const TimeBox = ({ value, label, max, isActive }: { value: number, label: string, max: number, isActive?: boolean }) => {
        const radius = 45; const circumference = 2 * Math.PI * radius; const progress = max === 0 ? 1 : value / max; const dashoffset = circumference - (progress * circumference); const formattedValue = value.toString().padStart(2, '0');
        return (
            <div className="flex flex-col items-center gap-2 relative group/box">
                <div className="relative w-full aspect-square max-w-[80px] sm:max-w-[100px] flex items-center justify-center">
                    <svg viewBox="0 0 100 100" className="w-full h-full transform -rotate-90 drop-shadow-sm">
                        ...
                        <circle cx="50" cy="50" r="45" className={`transition-all duration-1000 ease-linear ${isActive ? 'stroke-pink-500' : 'stroke-slate-300 dark:stroke-slate-600'}`} strokeWidth="6" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={dashoffset} fill="transparent" />
                    </svg>
                    <div className="absolute inset-0 flex items-center justify-center flex-col">
                        <span className={`text-xl sm:text-3xl font-black tabular-nums tracking-tighter ${isActive ? 'text-transparent bg-clip-text bg-gradient-to-br from-pink-500 to-rose-600 dark:from-pink-400 dark:to-rose-400' : 'text-slate-700 dark:text-slate-200'}`}>{formattedValue}</span>
                    </div>
                </div>
            ...
            <div className="relative bg-white/90 dark:bg-[#0B1121]/95 backdrop-blur-2xl rounded-[2rem] p-6 h-full flex flex-col justify-between overflow-hidden">
                ...
                <div className="flex flex-row items-center justify-between mb-6 z-10">
                    <div className="flex items-center gap-3">
                        <div className="p-2.5 bg-slate-50 dark:bg-white/5 rounded-xl text-pink-500 ring-1 ring-pink-500/20 shadow-sm"><Clock size={20} className="animate-[spin_4s_linear_infinite] opacity-80" /></div>
                        <div className="flex flex-col"><h3 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-[0.2em]">License Countdown</h3><p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mt-0.5">Aug 29, 2026</p></div>
                    </div>
                    <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-emerald-500/5 border border-emerald-500/20 rounded-full"><span className="relative flex h-1.5 w-1.5"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span><span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500"></span></span><span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Active</span></div>
                </div>
                <div className="grid grid-cols-4 gap-2 sm:gap-4 z-10">
                    <TimeBox value={timeLeft.days} label="Days" max={0} />
                ...
                <div className="mt-6 text-center z-10 border-t border-slate-100 dark:border-white/5 pt-4">
                    <p className="text-[10px] sm:text-xs font-medium text-slate-400 dark:text-slate-500 italic">"Every second brings you closer to that license."</p>
                </div>
            </div>
        </div>
    );
...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
      const hasSeen = localStorage.getItem('has_seen_dashboard_welcome_v1');
      if (!hasSeen) {
  ...
  const handleWelcomeClose = () => {
      localStorage.setItem('has_seen_dashboard_welcome_v1', 'true');
      setShowWelcomeModal(false);
  };
  
  const today = new Date();
  const priorityWeight: Record<TaskPriority, number> = { 'High': 3, 'Medium': 2, 'Low': 1 };
  const todaysTasks = allTasks.filter(t => { try { if (!t.start) return false; return isSameDay(new Date(t.start), today); } catch (e) { return false; } }).sort((a, b) => { if (a.completed !== b.completed) return a.completed ? 1 : -1; const weightA = priorityWeight[a.priority] || 0; const weightB = priorityWeight[b.priority] || 0; if (weightA !== weightB) return weightB - weightA; return new Date(a.start).getTime() - new Date(b.start).getTime(); });
                ...
                <div className={`${glassCard} relative p-8 flex items-center justify-between min-h-[180px] overflow-hidden`}>
                     ...
                     <div className="relative z-10 max-w-lg">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight leading-tight mb-2">
                            {greeting}, <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-500 dark:from-pink-400 dark:to-purple-400">{currentUser?.displayName?.split(' ')[0] || 'Student'}</span>,
                        </h1>
                        <p className="text-slate-500 dark:text-slate-400 text-sm md:text-base font-medium max-w-md">Ready to make progress? Your board exam journey continues today.</p>
                     </div>
                     <button onClick={() => onNavigate('Pomodoro Timer')} className="relative z-10 shrink-0 bg-slate-900 dark:bg-pink-600 hover:bg-slate-800 dark:hover:bg-pink-500 text-white shadow-2xl dark:shadow-[0_0_40px_rgba(236,72,153,0.4)] rounded-full h-16 w-16 md:w-auto md:px-8 md:py-4 font-bold transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2 group">
                ...
                <div className={`${glassCard} p-6 flex flex-col h-auto min-h-[300px] transition-all duration-300`}>
                    ...
                    <form onSubmit={handleQuickAdd} className="flex gap-3 mb-6 group/input shrink-0">
                        <div className="flex-1 relative">
                            <input id="quick-add-input" type="text" value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)} placeholder="Add a new task..." className="w-full px-5 py-3 bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-2xl focus:outline-none focus:border-pink-500 focus:ring-4 focus:ring-pink-500/10 text-slate-900 dark:text-white placeholder-slate-400 transition-all pl-12 text-sm font-medium" />
                            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within/input:text-pink-500 transition-colors"><Plus size={20} /></div>
                        </div>
                        <button type="submit" disabled={!newTaskText.trim()} className="px-6 bg-pink-600 text-white rounded-2xl hover:bg-pink-500 disabled:opacity-50 transition-all shadow-lg shadow-pink-500/20 font-bold text-sm">Add</button>
                    </form>
                    <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-2 min-h-[200px] max-h-[320px] scroll-smooth">
                        {todaysTasks.length === 0 ? <div className="h-full flex items-center justify-center py-8"><GhostEmptyState /></div> : todaysTasks.map(task => (
                ...
                <div className="flex-1 flex flex-col gap-4 min-h-[420px]">
                   <div className="flex items-center justify-between px-2 shrink-0">
                        ...
                        <div className="relative bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-xl flex gap-1 backdrop-blur-md border border-slate-200/50 dark:border-white/5">
                           <div className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white dark:bg-slate-700 rounded-lg shadow-sm transition-transform duration-300 ease-spring ${dailyWidgetTab === 'mnemonic' ? 'translate-x-0' : 'translate-x-full left-1'}`}></div>
                           <button onClick={() => setDailyWidgetTab('mnemonic')} className={`relative z-10 px-4 py-2 text-[10px] font-bold uppercase tracking-wider transition-colors duration-300 flex items-center gap-1.5 ${dailyWidgetTab === 'mnemonic' ? 'text-pink-600 dark:text-pink-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}><Lightbulb size={12} /> Mnemonic</button>
                           <button onClick={() => setDailyWidgetTab('recall')} className={`relative z-10 px-4 py-2 text-[10px] font-bold uppercase tracking-wider transition-colors duration-300 flex items-center gap-1.5 ${dailyWidgetTab === 'recall' ? 'text-purple-600 dark:text-purple-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'}`}><BrainCircuit size={12} /> Question</button>
                        </div>
                   </div>
