...
import { Dices, CheckCircle2, Loader2, Info, Brain, Zap, Stethoscope, ClipboardList, FileText, Stamp, Thermometer, User, Activity, AlertTriangle, X, History, Trash2, Calendar } from 'lucide-react';
...
const ClinicalRoulette: React.FC = () => {
    ...
    const [caseHistory, setCaseHistory] = useState<SavedCaseLog[]>(() => {
        const saved = localStorage.getItem('clinical_roulette_history');
        return saved ? JSON.parse(saved) : [];
    });

    useEffect(() => {
        localStorage.setItem('clinical_roulette_history', JSON.stringify(caseHistory));
    }, [caseHistory]);
        ...
        <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="text-center space-y-4 px-4 pt-4 relative">
                <button 
                    ...
                    className="absolute top-0 right-4 p-3 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-purple-500 transition-colors shadow-sm border border-slate-200 dark:border-slate-700"
                    title="Case History"
                >
                    <History size={20} />
                </button>
                    ...
                    <div className="bg-white dark:bg-slate-900 w-full max-w-3xl rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden flex flex-col max-h-[85vh] animate-zoom-in" onClick={e => e.stopPropagation()}>
                        <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="font-bold text-lg text-slate-900 dark:text-white">Case History</h3>
                                    <p className="text-xs text-slate-500">Log of your accepted scenarios.</p>
                                </div>
                            </div>
